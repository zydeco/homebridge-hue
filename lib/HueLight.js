...
// Transform bridge ct values [153 = 6500K, 500 = 2000K]
// to homekit hue value [0˚, 360˚] and saturation value [0%, 100%].
function ctHueSat(ct, modelid) {
  var hue, sat;
  if (ct < 305) {
    hue = ct <= 216 ? 180 + ((250 - ct) / 18.0) : 180;
    sat = 44 - (ct / 6.8);
  } else {
    hue = 45;
    if (ct <= 340) {
  ...
  return {hue: Math.round(hue), sat: Math.max(Math.round(sat),0)};
}

// Transform homekit hue value [0˚, 360˚] and saturation value [0%, 100%]
// to bridge ct values [153 = 6500K, 500 = 2000K].
function invCtHueSat(hue, sat, modelid) {
  var closestCt;
  var closestDistance = Infinity;
  var ctRange = sat < 10 ? [225, 340] : (hue < 112 || hue > 292) ? [340, 454] : [154, 225];
  for(var ct = ctRange[0]; ct < ctRange[1]; ct += 5) {
    var hs = ctHueSat(ct, modelid);
    var dh = Math.min(Math.abs(hs.hue-hue), 360-Math.abs(hs.hue-hue));
    var ds = Math.abs(sat-hs.sat);
    var d = dh*dh+ds*ds;
...
function HueLight(bridge, id, obj, type) {
  ...
  if (this.state.ct !== undefined) {
    ...
    if (this.bridge.platform.config.fakecolor && this.state.xy === undefined) {
      this.service.setCharacteristic(Characteristic.Hue, this.hk.fakehue);
      this.service.setCharacteristic(Characteristic.Saturation, this.hk.fakesat);
      this.service.getCharacteristic(Characteristic.Hue)
        .on("get", function(callback) {callback(null, this.hk.fakehue);}.bind(this))
        .on("set", this.setFakeHue.bind(this));
      this.service.getCharacteristic(Characteristic.Saturation)
        .on("get", function(callback) {callback(null, this.hk.fakesat);}.bind(this))
        .on("set", this.setFakeSat.bind(this));
    }
  }
...
HueLight.prototype.refresh = function() {
  ...
  if (this.state.ct !== undefined) {
    ...
    if (this.bridge.platform.config.fakecolor && this.state.xy === undefined) {
      const hs = ctHueSat(this.state.ct, this.obj.modelid);
      this.log.debug("%s: refreshing fake saturation: %s = %s°,%s%%", this.name, this.state.ct, hs.hue, hs.sat);
      this.hk.fakehue = hs.hue;
      this.hk.fakesat = hs.sat;
    }
  }
  if (this.state.xy !== undefined) {
...
HueLight.prototype.setFakeHue = function(hue, callback) {
  if (hue === this.hk.fakehue) {
    // light updated from hue bridge - we're good
    return callback();
  }
  this.log.info("%s: homekit fake hue changed from %s˚ to %s˚", this.name, this.hk.fakehue, hue);
  this.hk.fakehue = hue;
  
  const newCT = invCtHueSat(this.hk.fakehue, this.hk.fakesat, this.obj.modelid);
  this.log.debug("%s: ct for %s˚,%s%% = %d", this.name, this.hk.fakehue, this.hk.fakesat, newCT);
  return this.setCT(newCT, callback);
};

HueLight.prototype.setFakeSat = function(sat, callback) {
  if (sat === this.hk.fakesat) {
    // light updated from hue bridge - we're good
    return callback();
  }
  this.log.info("%s: homekit fake saturation changed from %s%% to %s%%", this.name,
  	        this.hk.fakesat, sat);
  this.hk.fakesat = sat;
